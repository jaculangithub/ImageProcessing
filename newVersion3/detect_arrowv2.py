...
if __name__ == "__main__":
    image = cv2.imread("./images/login.png")
    ...
    if arrow_image is not None:
        ...
        images_to_plot = [image_rgb, thresh_image, arrow_image]
        titles = ['Original Image', 'Threshold Image', 'Arrow Image']
        
        if arrow_info_image is not None:
            # Convert BGR to RGB for proper display
            arrow_info_image_rgb = cv2.cvtColor(arrow_info_image, cv2.COLOR_BGR2RGB)
            images_to_plot.append(arrow_info_image_rgb)
            titles.append('Arrow Info Image')
        
        plot_images(images_to_plot, titles, cmap='gray')
